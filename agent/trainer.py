...
def train_from_real_signals(symbol: str, db) -> Dict[str, Any]:
    """
    Treina sub-agente do símbolo usando sinais reais acumulados.
    Busca sinais com outcome do banco e cria episódios de replay.
    Args:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Treinamento RL para símbolo')
    parser.add_argument('--symbol', type=str, default='BTCUSDT', help='Símbolo para treinamento')
    parser.add_argument('--mode', type=str, default='paper', help='Modo de operação (paper/live)')
    parser.add_argument('--timesteps', type=int, default=500000, help='Timesteps para treinamento')
    args = parser.parse_args()

    root_logger = logging.getLogger()
    root_logger.info(f'Iniciando pipeline de treinamento para {args.symbol}...')

    db = DatabaseManager(DB_PATH)
    data_loader = DataLoader(db=db)
    train_data = data_loader.load_training_data(symbol=args.symbol, train_ratio=0.8, min_length=1000)

    signals = db.get_signals_for_training(symbol=args.symbol, limit=1000)
    if not signals:
        root_logger.warning(f'Nenhum sinal com outcome em trade_signals para {args.symbol}. Continuando com dados de OHLCV.')

    trainer = Trainer()
    trainer.train_phase1_exploration(train_data, total_timesteps=args.timesteps)
    root_logger.info('Treinamento concluído.')
