...
def test_order_layer_consumes_created_signal_without_real_order(tmp_path: Path) -> None:
    ...
    assert first["eligible_created_signals"] == 1
    assert first["consumed_now"] == 1
    assert first["cancelled_now"] == 0

    assert second["eligible_created_signals"] == 0
    assert second["consumed_now"] == 0
    assert second["cancelled_now"] == 0

    with sqlite3.connect(model2_db) as conn:
        row = conn.execute(
            "SELECT status, payload_json FROM technical_signals LIMIT 1"
        ).fetchone()
