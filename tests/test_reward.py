...
Valida a base do contrato atual de reward com componentes legados e expandidos.
"""
...
class TestRewardCalculator:
    ...
    def test_calculate_basic(self, reward_calc):
        """Testa cálculo básico de reward com componentes essenciais."""
        ...
        # Verifica que os componentes centrais e os retrocompatíveis estão presentes
        expected_keys = [
            'r_pnl',
            'r_hold_bonus',
            'r_invalid_action',
        ...
        # Componentes realmente obsoletos não devem reaparecer.
        old_keys = ['r_risk', 'r_consistency', 'r_overtrading']
        for key in old_keys:
    ...
    def test_hold_bonus_with_momentum(self, reward_calc):
        ...
        # Caso 3: Posição perdedora relevante (PnL <= -0.5)
        ...
        # hold_bonus = HOLD_LOSS_PENALTY * abs(pnl_pct)
        expected_bonus3 = -0.02 * abs(-2.5)
        assert result3['r_hold_bonus'] == pytest.approx(expected_bonus3, abs=0.01), \
            f"Hold bonus de posição perdedora deve ser {expected_bonus3}, recebeu {result3['r_hold_bonus']}"
